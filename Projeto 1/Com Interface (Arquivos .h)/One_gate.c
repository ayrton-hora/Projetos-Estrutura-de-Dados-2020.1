...
// Cash of park lot for stack structure
void cash_Stck (struct Stck_Vehicle* vehicle) { 
  float time = vehicle->time;
  while (time > -1) {
     if (time < 60) {
...
void main() {
  ...
  while (n != -1) {
    ...
    if (n == 9) {
      ...
      scanf("%d", &movements);
      push_stck(&root, plate, time, movements);
    }
    if (n == 10) {
      printf("\nPlease type the plate of vehicle: ");
      scanf("%s", plate);
      struct Stck_Vehicle* v = searchVehicle_Stck(&root, plate);
      cash_Stck(v);
      removeStckVehicle(&root, plate);
    }
    if (n == 11) {
      printf("\nVehicles:\n");
      printStack(root);
