...
int searchVehiclePos (struct DL_Vehicle** head, char *plate) { // Return the position of Vehicle v
  ...
  while (ref != NULL) { // Searching
    if (ref->plate == plate) {
      printf("Seaching...Vehicle %s found in the position: %d\n", ref->plate, ref->pos);
      return ref->pos;
    }
    ref = ref->next;
  }
...
void removeDLLVehicle (struct DL_Vehicle** head, char *plate) { // Remove a vehicle from your plate
  ...
  if (vehicle_pos <= half_lenght) {
    if (fst_list->plate == plate) { // If the vehicle is in the fst position
      printf("Vehicle %s found in the first position\n", plate);
      temp = fst_list;
      ...
      *head = fst_list->next;
      ...
      free(veh_removed);
      printf("Vehicle %s removed from the first position\n", plate);
      ...
      printf("Vehicle %s found in the position: %d\n", plate, fst_list->pos);
      veh_removed = fst_list;
      veh_removed->movements++;
      new_last1 = fst_list->next;
      new_last1->prev = NULL;
      printf("Vehicle %s removed from the position: %d\n", plate, veh_removed->pos);
      free(veh_removed);
...
void main() { // Main
  ...
  // removeDLLVehicle(&head, "0004"); // Unstill not 100% working
  
  printf("\nLenght of Doubly Linked List is: %d\n\n", length_DLL(&head));
